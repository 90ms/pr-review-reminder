...
- **토큰/비용 표기**: claude는 `--output-format json`의 실제 비용(`total_cost_usd`), codex는 `tokens used`.
...
- **리뷰 토큰 제한(세션 잔여량 기반)**: claude/codex CLI가 잔여 5시간 한도/토큰을 스크립트로
  노출하지 않아 **미구현(보류)**. 필요 시 히스토리 기반 로컬 예산 근사로 추가 가능.
