...
- **사람이 마지막 결정** — 코멘트, 승인, 피드백 이슈는 자동 게시하지 않습니다.
  피드백 이슈는 사용자가 분류 라벨을 고르면 `codex-ready`와 함께 생성됩니다.
...
- **기존 구독 활용** — 앱이 API 키를 받지 않고 로컬 `gh`, `claude`, `codex`
  로그인 상태를 사용합니다.
- **프로젝트 규칙 자동 적용** — 저장소의 리뷰 스킬, 컨벤션과 아키텍처 문서를
  찾아 분류하고, 경로 범위에 맞는 지침만 리뷰에 포함합니다.
...
앱 언어와 리뷰 출력 언어를 따로 선택할 수 있고, 팀의 리뷰 가이드라인을 직접 추가하거나
저장소에서 찾아 임포트할 수 있습니다. 매일 지정 시각 또는 일정 간격으로 수집하며 최근 실행 성공·실패를
...
여러 Draft PR은 저장소의 `$validate-github-prs` 스킬로 함께 검증할 수 있다.
