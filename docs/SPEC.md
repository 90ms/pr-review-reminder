...
5. 이미 분석이 끝난 PR에서 사용자가 **리뷰 다시하기**를 누르면 저장된 분석과
   상세 정보를 비우고 현재 head와 diff를 다시 가져와 분석한다.
6. 결과와 사용량을 히스토리에 저장하고, 설정에 따라 완료 알림을 보낸다.
...
| AC33 | 피드백 등록 후 입력 창을 닫고 입력 내용을 초기화하며, 등록한 이슈의 처리 상태를 피드백 히스토리에서 확인할 수 있다. |
| AC34 | 비정상 종료를 다음 실행에서 감지하되 중복 실행과 정상 종료는 충돌로 오인하지 않는다. |
| AC35 | 충돌 진단은 민감한 로그를 자동 첨부하거나 이슈를 자동 게시하지 않는다. |
