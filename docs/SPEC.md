...
이 데이터는 선택한 CLI의 외부 서비스 제공자에게 전송될 수 있다. 사용자는 리뷰 설정에서 단일 통합
프롬프트 방식 또는 베이스 프롬프트가 여러 자식 스킬 프롬프트 파일 내용을 참조하는
방식을 선택할 수 있다. 선택한 스킬 파일을 읽을 수 없거나 지침이 있는데 베이스
템플릿에 `{{SKILL}}`이 없으면 분석을 실행하지 않고 오류를 표시한다. 두 방식 모두 최종
AI 응답 형식은 동일하다. AI CLI는 빈 임시 작업 디렉터리와 최소 환경에서 실행한다.
