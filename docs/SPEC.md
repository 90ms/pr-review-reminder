...
2. 내가 작성한 PR에 도착한 미승인 리뷰 피드백을 별도 인박스에서 확인한다.
3. 필요한 PR만 AI로 분석해 요약, 리뷰 포인트, 인라인 코멘트 초안을 얻는다.
4. 큰 상세 화면에서 diff와 초안을 검토하고 편집한다.
5. 미리보기 후 사용자가 명시적으로 선택한 코멘트 또는 승인만 게시한다.
6. 같은 PR head commit의 결과는 로컬 히스토리에서 복원해 AI 사용량을 줄인다.
...
`gh`, `claude`, `codex` 설치 및 인증 상태를 진단한다. 설정은 논리적 탭으로
구분하며 GitHub owner, 선택 저장소, AI 도구, 언어, 스케줄, 알림, 프롬프트와
리뷰 가이드라인을 지정한다.

### 3.2 수집과 분석

1. 사용자가 새로고침하거나 앱 실행 중 스케줄이 도래한다.
2. 앱은 `gh search prs --review-requested=@me --state=open`으로 내가 리뷰할 PR을
   수집한다.
3. 앱은 `gh search prs --author=@me --state=open` 후보의 `reviewDecision`과 정식
   review를 제한된 동시 조회로 확인해 내 PR 피드백 인박스를 갱신한다.
4. 동일한 PR/head SHA의 히스토리가 있으면 저장된 결과를 복원한다.
5. 사용자가 **코드 리뷰**를 누르거나 자동 리뷰 설정이 켜져 있으면 PR 본문과
   diff를 가져와 선택한 AI CLI로 분석한다.
6. 이미 분석이 끝난 PR에서 사용자가 **리뷰 다시하기**를 누르면 저장된 분석과
   상세 정보를 비우고 현재 head와 diff를 다시 가져와 분석한다.
7. 결과와 사용량을 히스토리에 저장하고, 설정에 따라 완료 알림을 보낸다.
...
- 피드백은 AI로 제목/본문을 정돈할 수 있다. 사용자는 `bug`, `enhancement`,
  `question` 중 하나의 분류 라벨을 선택한다. 제출 액션으로만
  `90ms/pr-review-reminder` GitHub 이슈를 만들며, 모든 피드백 이슈에는
  `codex-ready`와 선택한 분류 라벨을 붙인다. 라벨 적용이 실패하면 이슈 생성은
  라벨 없이 한 번 재시도한다. `gh`를 사용할 수 없으면 실행할 명령만 미리보기로
  보여준다. 등록된 피드백 이슈는 로컬 히스토리에 저장하고, 사용자가 새로고침하면
  `gh issue view`로 open/closed/resolved 상태를 확인한다. 개별 이슈 조회 실패는
  표시하되 다른 이슈의 상태 갱신은 계속한다.
...
- 수집: `gh search` 내부 pagination으로 owner 전체 또는 선택 저장소의 리뷰 요청 PR과
  내가 작성한 열린 PR 후보를 각각 GitHub Search 상한인 최대 1,000개까지 조회한다.
- 내 PR 피드백: 후보 PR은 `reviewDecision != APPROVED`이고 `COMMENTED`,
  `CHANGES_REQUESTED`, `APPROVED` 중 하나의 정식 PR review가 있을 때만 표시한다.
  `CHANGES_REQUESTED`가 있으면 변경 요청됨, 최신 review가 `COMMENTED`이면 코멘트
  도착, 그 외 미승인 항목은 승인 대기로 표시한다.
...
| AC30 | 완료된 PR에서 사용자가 현재 head와 diff를 다시 가져와 리뷰를 재실행할 수 있다. |
| AC31 | 설정 항목을 논리적인 탭과 섹션으로 구분해 표시한다. |
| AC32 | 로그인 시 실행을 설정하고 진행 중 작업 종료 전에 사용자 확인을 받는다. |
| AC33 | 비정상 종료를 다음 실행에서 감지하되 중복 실행과 정상 종료는 충돌로 오인하지 않는다. |
...
| AC38 | 피드백 이슈 생성 시 `codex-ready`와 사용자가 선택한 `bug`/`enhancement`/`question` 분류 라벨을 붙이고, 라벨 적용 실패 시 이슈 생성 자체는 완료한다. |
...
- PR 인박스는 `review-requested:@me` 리뷰 요청과 `author:@me` 미승인 리뷰 피드백을
  분리해서 수집한다. 피드백 인박스는 정식 PR review만 포함하며 GitHub의 현재
  `reviewDecision`을 승인 여부의 기준으로 사용한다.
