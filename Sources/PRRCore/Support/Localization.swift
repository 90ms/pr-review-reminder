import Foundation

public enum AppLanguage: String, Sendable, Codable, CaseIterable, Identifiable {
    case system
    case korean
    case english

    public var id: String { rawValue }

    /// Resolves to a concrete language code ("ko"/"en"). `.system` follows the
    /// provided locale (defaults to the current locale).
    public func resolved(locale: Locale = .current) -> String {
        switch self {
        case .korean: return "ko"
        case .english: return "en"
        case .system:
            let code = locale.language.languageCode?.identifier ?? "en"
            return code == "ko" ? "ko" : "en"
        }
    }

    /// Instruction appended to AI prompts to control review output language.
    public func promptDirective(locale: Locale = .current) -> String {
        resolved(locale: locale) == "ko"
            ? "Write the summary, reviewPoints, and inlineComments in Korean."
            : "Write the summary, reviewPoints, and inlineComments in English."
    }
}

/// Minimal in-code localization. Returns the key itself when a string is missing
/// so a forgotten key is visible rather than crashing.
public struct L10n: Sendable {
    public let code: String   // "ko" or "en"

    public init(language: AppLanguage, locale: Locale = .current) {
        self.code = language.resolved(locale: locale)
    }

    public func callAsFunction(_ key: String) -> String {
        Self.table[code]?[key] ?? Self.en[key] ?? key
    }

    static let en: [String: String] = [
        "refresh": "Refresh now",
        "setup_needed": "Setup needed",
        "nothing_awaiting": "Nothing awaiting your review.",
        "nothing_feedback": "No unapproved feedback on your PRs.",
        "no_data": "No data yet — hit refresh.",
        "inbox_review_requests": "PRs to review",
        "inbox_my_feedback": "My PR feedback",
        "updated": "Updated",
        "next_run": "Next",
        "settings": "Settings",
        "quit": "Quit",
        "feedback": "Leave feedback",
        "analyzing": "Analyzing…",
        "queued": "Queued",
        "analysis_failed": "Analysis failed",
        "cancel_review": "Cancel",
        "review_cancelled": "Review cancelled.",
        "review_timed_out": "Review timed out. Try again.",
        "review_points": "Review points",
        "severity": "Severity",
        "severity_high": "High",
        "severity_medium": "Medium",
        "severity_low": "Low",
        "inline_comments": "Inline comments",
        "remove_inline_comment": "Remove inline comment",
        "no_inline_comments_to_post": "No inline comments selected.",
        "post_comments": "Post comments",
        "summary": "Summary",
        "approve": "Approve",
        "approve_no_issues": "No issues · Approve",
        "approve_no_issues_body": "I found no issues requiring inline comments. Approving.",
        "open_github": "Open on GitHub",
        "view_detail": "Details",
        "run_review": "Run code review",
        "retry": "Retry",
        "not_reviewed": "Not reviewed yet.",
        "auto_review": "Auto-review PRs when found",
        "prompt_help": "Instruction sent to the AI. {{TITLE}}, {{BODY}}, {{DIFF}}, {{SKILL}} are replaced with real values.",
        "skill_help": "Extra reviewer rules/guidelines injected into the prompt at {{SKILL}}.",
        "diff": "Diff",
        "diff_truncated_warning": "AI review uses only the first 60,000 characters of this diff.",
        "loading_diff": "Loading changes…",
        "diff_load_failed": "Could not load changes",
        "diff_empty": "No textual changes to display.",
        "detail_layout": "Detail layout",
        "detail_review": "Review",
        "detail_changes": "Changes",
        "detail_side_by_side": "Side by side",
        "diff_split": "Split",
        "diff_unified": "Unified",
        "cost": "Cost",
        "tokens_unit": "tokens",
        "pr_detail": "Pull request",
        "some_failed": "Some comments failed to post:",
        "submit": "Submit",
        "cancel": "Cancel",
        "preview_inline": "These inline comments will be posted:",
        "preview_summary": "This summary will be posted as a comment:",
        "preview_approve": "This pull request will be approved.",
        "approve_body_ph": "Approval comment (optional)",
        "caution_high": "⚠️ %d high-severity review point(s) found — approve anyway?",
        "no_high": "No high-severity review points.",
        "target": "Target",
        "posting": "Posting…",
        "review_done_title": "Review complete",
        "review_done_body": "Review finished for %@",
        "pr_feedback_title": "PR feedback",
        "pr_feedback_body": "%d new review feedback item(s) on your PRs.",
        "feedback_changes_requested": "Changes requested",
        "feedback_commented": "Comment received",
        "feedback_awaiting_approval": "Awaiting approval",
        "post_and_approve": "Comment & approve",
        "approve_after_comments_body": "Left a few comments, but they're non-blocking — safe to merge. Approving.",
        "history": "History",
        "history_empty": "No reviews recorded yet.",
        "history_detail": "Review history",
        "review_again": "Review again",
        "review_completed": "Review complete.",
        "total_usage": "Total usage",
        "reviews_count": "%d reviews",
        "delete": "Delete",
        "restored": "Restored from history (no tokens used)",
        "sec_history": "Local history",
        "save_history": "Save review history locally",
        "retention_forever": "Retention: until manually deleted",
        "retention_days": "Retention: %d days",
        "history_privacy_help": "History contains PR text, diffs, and AI output in Application Support.",
        "delete_all_history": "Delete all history",
        "delete_all_history_confirm": "Permanently delete all locally saved review history?",
        "codex_input_price": "Codex input USD / 1M tokens",
        "codex_output_price": "Codex output USD / 1M tokens",
        "codex_price_help": "Codex reports total tokens only. Input/output and cost are local estimates; set current model prices.",
        "sec_budget": "Local review budget",
        "settings_tab_general": "General",
        "settings_tab_review": "Review",
        "settings_tab_automation": "Automation",
        "settings_tab_data": "Data",
        "settings_tab_advanced": "Advanced",
        "token_budget": "Token limit (0 = disabled)",
        "budget_window_days": "Rolling window: %d days",
        "budget_help": "Blocks new AI reviews when recorded history reaches the limit. Publishing remains available.",
        "review_budget_exceeded": "Local review budget reached: %@ / %@ tokens in the last %d days.",
        // settings
        "sec_github": "GitHub",
        "owner": "Owner / org",
        "repos_ph": "Repos (comma-separated, empty = all)",
        "sec_ai": "AI tool",
        "tool": "Tool",
        "sec_schedule": "Schedule",
        "mode": "Mode",
        "daily_at": "Daily at",
        "every_n": "Every N hours",
        "hour": "Hour",
        "minute": "Min",
        "every_h": "Every %d h",
        "sec_lang": "Language",
        "sec_app_behavior": "App behavior",
        "sec_automatic_review": "Automatic review",
        "app_language": "App language",
        "review_language": "Review language",
        "lang_system": "System",
        "lang_korean": "한국어",
        "lang_english": "English",
        "enable_notifications": "Enable notifications",
        "sec_prompt": "Prompt template",
        "sec_skill": "Review skill / guidelines",
        "load_file": "Load from file…",
        "add_skill_files": "Add guideline files…",
        "skill_files_empty": "No guideline files selected.",
        "repository_guidelines": "Repository guidelines",
        "guideline_repository_ph": "Repository (owner/name)",
        "discover_guidelines": "Find in repository",
        "discovering_guidelines": "Searching and classifying Markdown documents…",
        "guidelines_imported": "Imported %d guideline document(s).",
        "imported_guidelines_empty": "No repository guideline documents imported.",
        "guideline_repository_required": "Enter a repository as owner/name.",
        "guideline_ai_unavailable": "%@ CLI is not available.",
        "clear": "Clear",
        "sec_deps": "Dependencies",
        "recheck": "Re-check",
        "gh_installed": "gh installed",
        "gh_authed": "GitHub authentication",
        "claude_cli": "claude CLI",
        "codex_cli": "codex CLI",
        "save": "Save",
        "not_checked": "Not checked yet.",
        "sec_update": "Updates",
        "current_version": "Current version",
        "latest_version": "Latest version",
        "check_update": "Check for updates",
        "checking_update": "Checking…",
        "install_update": "Install update",
        "installing_update": "Building and installing the update…",
        "up_to_date": "Up to date",
        "update_homebrew_only": "In-app updates are available for Homebrew installations.",
        "update_restart": "Update installed. Restarting the app…",
        "update_stage_refresh": "Refreshing the Homebrew tap…",
        "update_stage_version": "Reading version information…",
        "update_stage_build": "Building and installing the update…",
        "update_stage_link": "Updating the Applications link…",
        "update_failed": "Update failed",
        "update_failed_refresh": "Could not refresh the Homebrew tap",
        "update_failed_version": "Could not read version information",
        "update_failed_build": "Could not build or install the update",
        "update_failed_link": "The update installed, but the Applications link could not be updated",
        "update_stage_restart": "Opening the updated application…",
        "update_failed_restart": "The update installed, but the application could not be restarted",
        "update_cancelled": "Update operation cancelled.",
        "update_stage_cancelling": "Cancelling the Homebrew operation…",
        "restart_now": "Restart now",
        "sec_storage": "Local storage",
        "settings_storage": "Settings",
        "history_storage": "Review history",
        "storage_healthy": "Healthy",
        "storage_empty": "No saved data yet",
        "storage_read_failed": "Read failed",
        "storage_decode_failed": "Saved data was corrupt and defaults were restored",
        "storage_write_failed": "Write failed; the in-memory data is still available",
        "last_saved": "Last saved:",
        "corrupt_backup": "Corrupt data backup:",
        "search_limit_warning": "Search reached GitHub's 1,000-result limit.",
        "github_retried": "GitHub reads retried %d time(s).",
        "filter_files": "Filter changed files",
        "previous_change": "Previous change",
        "next_change": "Next change",
        "recent_schedule_runs": "Recent scheduled runs",
        "scheduled_refresh_succeeded": "%d pull request(s) found",
        "scheduled_refresh_failed": "Scheduled refresh failed",
        "scheduled_refresh_unknown_error": "The refresh failed without an error message.",
        "launch_at_login": "Launch at login",
        "launch_at_login_failed": "Could not change login item: %@",
        "quit_while_busy": "Work is still in progress",
        "quit_while_busy_help": "Quitting now cancels the active refresh, review, detail load, or update.",
        "quit_anyway": "Quit anyway",
        "unexpected_termination_title": "The previous session ended unexpectedly",
        "unexpected_termination_message": "Would you like to review a diagnostic draft and optionally submit it as a GitHub issue?",
        "report_issue": "Review issue",
        "dismiss": "Dismiss",
        // feedback window
        "fb_title": "Title",
        "fb_body": "Details",
        "fb_label": "Label",
        "fb_label_bug": "Bug",
        "fb_label_enhancement": "Feature",
        "fb_label_question": "Question",
        "fb_tidy": "Tidy with agent",
        "fb_submit": "Submit as issue",
        "fb_cancel": "Cancel",
        "fb_preview": "Command preview (not executed)",
        "fb_hold": "GitHub CLI is unavailable, so the issue was not created. The command that would run is shown below.",
        "fb_tidying": "Tidying…",
        "fb_history": "Feedback history",
        "fb_history_empty": "No feedback issues submitted yet.",
        "fb_refresh": "Refresh",
        "fb_open": "Open",
        "fb_closed": "Closed",
        "fb_resolved": "Resolved",
        "fb_unknown": "Unknown",
        "fb_labels_omitted": "The issue was created, but GitHub rejected these labels: %@. Add them manually to start the approval automation.",
    ]

    static let ko: [String: String] = [
        "refresh": "지금 새로고침",
        "setup_needed": "설정 필요",
        "nothing_awaiting": "리뷰 대기 중인 PR이 없습니다.",
        "nothing_feedback": "내 PR의 미승인 피드백이 없습니다.",
        "no_data": "아직 데이터 없음 — 새로고침하세요.",
        "inbox_review_requests": "내가 리뷰할 PR",
        "inbox_my_feedback": "내 PR 피드백",
        "updated": "업데이트",
        "next_run": "다음 실행",
        "settings": "설정",
        "quit": "종료",
        "feedback": "의견 남기기",
        "analyzing": "분석 중…",
        "queued": "대기 중",
        "analysis_failed": "분석 실패",
        "cancel_review": "취소",
        "review_cancelled": "리뷰가 취소됐습니다.",
        "review_timed_out": "리뷰 시간이 초과됐습니다. 다시 시도하세요.",
        "review_points": "리뷰 포인트",
        "severity": "심각도",
        "severity_high": "높음",
        "severity_medium": "보통",
        "severity_low": "낮음",
        "inline_comments": "인라인 코멘트",
        "remove_inline_comment": "인라인 코멘트 제거",
        "no_inline_comments_to_post": "등록할 인라인 코멘트가 없습니다.",
        "post_comments": "코멘트 등록",
        "summary": "요약",
        "approve": "승인",
        "approve_no_issues": "문제 없음 · 승인",
        "approve_no_issues_body": "인라인으로 남길 문제가 없어 승인합니다.",
        "open_github": "GitHub에서 열기",
        "view_detail": "자세히 보기",
        "run_review": "코드 리뷰",
        "retry": "다시 시도",
        "not_reviewed": "아직 리뷰하지 않음.",
        "auto_review": "PR 발견 시 자동 코드리뷰",
        "prompt_help": "AI에게 보내는 지시문. {{TITLE}}, {{BODY}}, {{DIFF}}, {{SKILL}}가 실제 값으로 치환됩니다.",
        "skill_help": "프롬프트의 {{SKILL}} 위치에 주입되는 추가 리뷰 규칙/가이드라인입니다.",
        "diff": "변경 내용",
        "diff_truncated_warning": "AI 리뷰에는 이 diff의 앞 60,000자만 사용됩니다.",
        "loading_diff": "변경 내용 불러오는 중…",
        "diff_load_failed": "변경 내용을 불러오지 못했습니다",
        "diff_empty": "표시할 텍스트 변경 내용이 없습니다.",
        "detail_layout": "상세 화면 구성",
        "detail_review": "리뷰",
        "detail_changes": "변경 내용",
        "detail_side_by_side": "나란히",
        "diff_split": "분할",
        "diff_unified": "통합",
        "cost": "비용",
        "tokens_unit": "토큰",
        "pr_detail": "풀 리퀘스트",
        "some_failed": "일부 코멘트 등록 실패:",
        "submit": "제출",
        "cancel": "취소",
        "preview_inline": "다음 인라인 코멘트가 등록됩니다:",
        "preview_summary": "다음 요약이 코멘트로 등록됩니다:",
        "preview_approve": "이 풀 리퀘스트를 승인합니다.",
        "approve_body_ph": "승인 코멘트 (선택)",
        "caution_high": "⚠️ 높은 심각도 리뷰 포인트 %d건 — 그래도 승인할까요?",
        "no_high": "높은 심각도 리뷰 포인트 없음.",
        "target": "대상",
        "posting": "등록 중…",
        "review_done_title": "리뷰 완료",
        "review_done_body": "%@ 리뷰가 완료됐습니다.",
        "pr_feedback_title": "PR 피드백",
        "pr_feedback_body": "내 PR에 새 리뷰 피드백 %d건이 있습니다.",
        "feedback_changes_requested": "변경 요청됨",
        "feedback_commented": "코멘트 도착",
        "feedback_awaiting_approval": "승인 대기",
        "post_and_approve": "코멘트 남기고 승인",
        "approve_after_comments_body": "코멘트를 남겼지만 머지되어도 문제없는 내용입니다. 승인합니다.",
        "history": "히스토리",
        "history_empty": "저장된 리뷰 기록이 없습니다.",
        "history_detail": "리뷰 히스토리",
        "review_again": "리뷰 다시하기",
        "review_completed": "리뷰가 완료됐습니다.",
        "total_usage": "누적 사용량",
        "reviews_count": "리뷰 %d건",
        "delete": "삭제",
        "restored": "히스토리에서 복원됨 (토큰 미사용)",
        "sec_history": "로컬 히스토리",
        "save_history": "리뷰 히스토리를 로컬에 저장",
        "retention_forever": "보존: 직접 삭제할 때까지",
        "retention_days": "보존: %d일",
        "history_privacy_help": "히스토리에는 PR 본문, diff, AI 결과가 Application Support에 저장됩니다.",
        "delete_all_history": "히스토리 전체 삭제",
        "delete_all_history_confirm": "로컬에 저장된 리뷰 히스토리를 모두 영구 삭제할까요?",
        "codex_input_price": "Codex 입력 USD / 100만 토큰",
        "codex_output_price": "Codex 출력 USD / 100만 토큰",
        "codex_price_help": "Codex는 총 토큰만 보고합니다. 입출력 구분과 비용은 로컬 추정치이며 현재 모델 단가를 입력해야 합니다.",
        "sec_budget": "로컬 리뷰 예산",
        "settings_tab_general": "일반",
        "settings_tab_review": "리뷰",
        "settings_tab_automation": "자동화",
        "settings_tab_data": "데이터",
        "settings_tab_advanced": "고급",
        "token_budget": "토큰 한도 (0 = 사용 안 함)",
        "budget_window_days": "집계 기간: 최근 %d일",
        "budget_help": "저장된 히스토리가 한도에 도달하면 새 AI 리뷰를 막습니다. 기존 결과 게시는 계속 가능합니다.",
        "review_budget_exceeded": "로컬 리뷰 예산 도달: 최근 %3$d일간 %1$@ / %2$@ 토큰.",
        // settings
        "sec_github": "GitHub",
        "owner": "Owner / org",
        "repos_ph": "레포 (쉼표 구분, 비우면 전체)",
        "sec_ai": "AI 도구",
        "tool": "도구",
        "sec_schedule": "스케줄",
        "mode": "방식",
        "daily_at": "매일 지정 시각",
        "every_n": "N시간 간격",
        "hour": "시",
        "minute": "분",
        "every_h": "%d시간마다",
        "sec_lang": "언어",
        "sec_app_behavior": "앱 동작",
        "sec_automatic_review": "자동 리뷰",
        "app_language": "앱 언어",
        "review_language": "리뷰 언어",
        "lang_system": "시스템",
        "lang_korean": "한국어",
        "lang_english": "English",
        "enable_notifications": "알림 사용",
        "sec_prompt": "프롬프트 템플릿",
        "sec_skill": "리뷰 스킬 / 가이드라인",
        "load_file": "파일에서 불러오기…",
        "add_skill_files": "가이드라인 파일 추가…",
        "skill_files_empty": "선택한 가이드라인 파일이 없습니다.",
        "repository_guidelines": "저장소 가이드라인",
        "guideline_repository_ph": "저장소 (owner/name)",
        "discover_guidelines": "저장소에서 찾아보기",
        "discovering_guidelines": "Markdown 문서를 검색하고 분류하는 중…",
        "guidelines_imported": "가이드라인 문서 %d개를 임포트했습니다.",
        "imported_guidelines_empty": "임포트한 저장소 가이드라인 문서가 없습니다.",
        "guideline_repository_required": "저장소를 owner/name 형식으로 입력하세요.",
        "guideline_ai_unavailable": "%@ CLI를 사용할 수 없습니다.",
        "clear": "지우기",
        "sec_deps": "의존성",
        "recheck": "다시 확인",
        "gh_installed": "gh 설치됨",
        "gh_authed": "GitHub 인증",
        "claude_cli": "claude CLI",
        "codex_cli": "codex CLI",
        "save": "저장",
        "not_checked": "아직 확인 안 함.",
        "sec_update": "업데이트",
        "current_version": "현재 버전",
        "latest_version": "최신 버전",
        "check_update": "업데이트 확인",
        "checking_update": "확인 중…",
        "install_update": "업데이트 설치",
        "installing_update": "업데이트를 빌드하고 설치하는 중…",
        "up_to_date": "최신 버전입니다",
        "update_homebrew_only": "앱 내 업데이트는 Homebrew 설치본에서 사용할 수 있습니다.",
        "update_restart": "업데이트를 설치했습니다. 앱을 다시 시작하는 중…",
        "update_stage_refresh": "Homebrew tap을 갱신하는 중…",
        "update_stage_version": "버전 정보를 확인하는 중…",
        "update_stage_build": "업데이트를 빌드하고 설치하는 중…",
        "update_stage_link": "Applications 링크를 갱신하는 중…",
        "update_failed": "업데이트 실패",
        "update_failed_refresh": "Homebrew tap을 갱신하지 못했습니다",
        "update_failed_version": "버전 정보를 확인하지 못했습니다",
        "update_failed_build": "업데이트를 빌드하거나 설치하지 못했습니다",
        "update_failed_link": "업데이트는 설치했지만 Applications 링크를 갱신하지 못했습니다",
        "update_stage_restart": "업데이트된 앱을 여는 중…",
        "update_failed_restart": "업데이트는 설치했지만 앱을 다시 시작하지 못했습니다",
        "update_cancelled": "업데이트 작업을 취소했습니다.",
        "update_stage_cancelling": "Homebrew 작업을 취소하는 중…",
        "restart_now": "지금 다시 시작",
        "sec_storage": "로컬 저장소",
        "settings_storage": "설정",
        "history_storage": "리뷰 히스토리",
        "storage_healthy": "정상",
        "storage_empty": "아직 저장된 데이터가 없습니다",
        "storage_read_failed": "읽기 실패",
        "storage_decode_failed": "저장 데이터가 손상되어 기본값으로 복원했습니다",
        "storage_write_failed": "쓰기 실패 — 메모리의 데이터는 유지됩니다",
        "last_saved": "마지막 저장:",
        "corrupt_backup": "손상 데이터 백업:",
        "search_limit_warning": "GitHub 검색 결과가 1,000건 상한에 도달했습니다.",
        "github_retried": "GitHub 조회를 %d회 재시도했습니다.",
        "filter_files": "변경 파일 검색",
        "previous_change": "이전 변경",
        "next_change": "다음 변경",
        "recent_schedule_runs": "최근 예약 실행",
        "scheduled_refresh_succeeded": "PR %d개 확인",
        "scheduled_refresh_failed": "예약 새로고침 실패",
        "scheduled_refresh_unknown_error": "오류 메시지 없이 새로고침에 실패했습니다.",
        "launch_at_login": "로그인 시 실행",
        "launch_at_login_failed": "로그인 항목을 변경하지 못했습니다: %@",
        "quit_while_busy": "작업이 진행 중입니다",
        "quit_while_busy_help": "지금 종료하면 진행 중인 새로고침, 리뷰, 상세 조회 또는 업데이트가 취소됩니다.",
        "quit_anyway": "그래도 종료",
        "unexpected_termination_title": "이전 실행이 비정상적으로 종료되었습니다",
        "unexpected_termination_message": "진단 초안을 확인한 뒤 GitHub 이슈로 등록할까요?",
        "report_issue": "이슈 검토",
        "dismiss": "닫기",
        // feedback window
        "fb_title": "제목",
        "fb_body": "내용",
        "fb_label": "라벨",
        "fb_label_bug": "버그",
        "fb_label_enhancement": "기능",
        "fb_label_question": "문의",
        "fb_tidy": "에이전트로 정돈",
        "fb_submit": "이슈로 등록",
        "fb_cancel": "취소",
        "fb_preview": "커맨드 미리보기 (실행 안 됨)",
        "fb_hold": "GitHub CLI를 사용할 수 없어 이슈를 만들지 않았습니다. 실행될 커맨드는 아래와 같습니다.",
        "fb_tidying": "정돈 중…",
        "fb_history": "의견 히스토리",
        "fb_history_empty": "아직 등록한 의견 이슈가 없습니다.",
        "fb_refresh": "새로고침",
        "fb_open": "열림",
        "fb_closed": "닫힘",
        "fb_resolved": "해결됨",
        "fb_unknown": "알 수 없음",
        "fb_labels_omitted": "이슈는 생성됐지만 GitHub가 다음 라벨을 거부했습니다: %@. 승인 자동화를 시작하려면 라벨을 직접 추가하세요.",
    ]

    static let table: [String: [String: String]] = ["en": en, "ko": ko]
}
